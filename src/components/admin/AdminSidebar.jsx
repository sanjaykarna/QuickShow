    ...
    max-w-13 md:max-w-60 w-full border-r border-gray-300/20 text-sm'>
        ...
        <div className='w-full'>
            {adminNavlinks.map((link, index)=>(
                <NavLink key={index} to={link.path} end className={({isActive})=>`relative flex items-center max-md:justify-center gap-2 w-full py-2.5 min-md:pl-10 first:mt-6 text-gray-400 ${isActive && 'bg-red-400/15 text-primary group'}`}>
                {({isActive})=>(
                    <>
